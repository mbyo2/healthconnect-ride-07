    ...
    <div className="space-y-4 font-sans text-slate-900 dark:text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center border-b border-canvas-silk pb-3">
        ...
        <div className="flex gap-2">
          <button onClick={() => { refresh(); refreshRequests(); }} className="px-3 py-1.5 rounded-md bg-canvas-mist dark:bg-slate-800 font-bold text-xs flex items-center gap-1">
      ...
      <div className="flex items-center gap-1.5 p-1 bg-white dark:bg-slate-900 border border-canvas-silk rounded-xl">
        {(["inventory", "requests"] as const).map((tab) => (
          <button
            ...
            className={`px-3.5 py-1.5 rounded-md text-xs font-extrabold capitalize transition-all ${activeTab === tab ? "bg-primary-500 text-white shadow-xs" : "text-graphite-500 dark:text-slate-400 hover:bg-canvas-mist dark:hover:bg-slate-800"}`}
          >
