...
export const IoTHealthMonitoring: React.FC<IoTHealthMonitoringProps> = ({ patientId }) => {
  ...
  const loadIoTData = async () => {
    try {
      ...
      // Load connected devices - using getPatientMetrics to get devices indirectly
      const patientMetrics = await iotHealthMonitoring.getPatientMetrics(patientId, undefined, '24h');
      setMetrics(patientMetrics || []);
      
      // Extract unique devices from metrics
      const uniqueDevices = Array.from(new Set(patientMetrics?.map(m => m.deviceId) || [])).map(deviceId => ({
        id: deviceId,
        patientId,
        deviceType: 'unknown',
  ...
  const calibrateDevice = async (deviceId: string) => {
    try {
      await iotHealthMonitoring.calibrateDevice(
        deviceId,
        'heart_rate', // metricType
        100, // referenceValue
        98 // deviceValue
      );
  ...
  const acknowledgeAlert = async (alertId: string) => {
    try {
      await iotHealthMonitoring.acknowledgeAlert(alertId, 'user_id');
