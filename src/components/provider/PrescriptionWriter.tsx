...
export const PrescriptionWriter = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // NOTE: comprehensive_prescriptions.status is CHECK-constrained to
      // pending/filled/partially_filled/cancelled/expired - 'active' would 400.
      // prescribed_date is a DATE column; prescription_number is generated by
      // the DB trigger (generate_prescription_number), so we read it back.
      const prescribedDate = new Date().toISOString().slice(0, 10); // YYYY-MM-DD
      ...
      const rxNumber = (created as any[])?.[0]?.prescription_number || 'issued';
