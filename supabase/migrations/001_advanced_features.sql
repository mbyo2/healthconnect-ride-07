...
-- This migration adds new tables and columns for IoT, Analytics, and Gamification features
-- Safe to run on existing database - uses IF NOT EXISTS and ADD COLUMN IF NOT EXISTS
...
-- Daily health metrics table (renamed to avoid conflict with existing health_metrics)
...
CREATE INDEX IF NOT EXISTS idx_daily_health_metrics_user_date ON public.daily_health_metrics(user_id, date DESC);
...
COMMENT ON TABLE public.daily_health_metrics IS 'Daily health metrics aggregated data';
